...
with tab2:
    ...
    if submitted_bolzen and bolzen_menge > 0:
        ...
        with col1:
            ...
            st.markdown(f"**Umsatz:** {umsatz_barren_wunsch:.2f} G")
            st.markdown("**Gewinn:** " + (f"<span style='color:green;font-weight:bold;'>{gewinn_barren_wunsch:.2f} G</span>" if gewinn_barren_wunsch >= 0 else f"<span style='color:red;font-weight:bold;'>{gewinn_barren_wunsch:.2f} G</span>"), unsafe_allow_html=True)
